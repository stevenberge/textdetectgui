...
#define DisIMG(qlabel, tmp)  cv::cvtColor(tmp, tmp, CV_BGR2RGB);\
...
void run(MainWindow &mw, char *filename, bool debug){
    ...
    for (int step =1; step<3; step++)
    {
        ...
        for(int iii = 0; iii<2; iii++){
            ...
            cout<<"after inited msers"<<endl;
            {
                ...
                const QString qs = "frame1";
              //  cv::cvtColor(tmp, tmp, CV_BGR2RGB);
           //      QImage tt =   QImage((const unsigned char*)(tmp.data),tmp.cols,tmp.rows, tmp.cols*tmp.channels(),  QImage::Format_RGB888);
              QLabel *qf1 = mw.findChild<QLabel*>(qs);
               DisIMG(qf1, tmp);
            //    qf1->setPixmap(QPixmap::fromImage(tt));
                imwrite(buf, tmp);
            }
                ...
                for(int i=0; i<lines.size(); i++){
                    ...
                    if(n>1)
                    {
                        IFOPT{
                            ...
                            char buf[100]; sprintf(buf, "out1/%s.%d.%d.%d.group.png",  filename, step, iii, i);
                            imwrite(buf, tmp);
                        }
...
//                    if(valid_num[step-1][i]) tv.push_back(i);
                ...
                QLabel *qf2 = mw.findChild<QLabel*>("frame2"); DisIMG(qf2, tmp);
                imwrite(buf, tmp);
            }
        }
        ...
        //////////////////////////////////////////
        {
            ...
            cv::cvtColor(tmp, tmp, CV_GRAY2BGR);
            QLabel *qf3 = mw.findChild<QLabel*>("frame3"); DisIMG(qf3, tmp);
            imwrite(buf, tmp);
        }
    }
    ////////////////////////////////////////////
