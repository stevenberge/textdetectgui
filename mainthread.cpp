...
#define IFOPT   if(debug)
  ...
  for (int step =1; step<3; step++)
  {
    ...
    for(int iii = 0; iii<=3; iii++){
     ...
     emit setLabelSig(step, thrs[iii]);
      ...
      std::cout<<"step:"<<step<<"."<<iii<<endl;
      std::cout<<"mser"<<endl;
      mser8((uchar*)grey.data, grey.cols, grey.rows, regions);
        std::cout<<regions.size()<<" regions extracted"<<endl;
      for (int i=0; i<regions.size(); i++){
        regions[i].er_fill(grey);
        regions[i].extract_features(lab_img, grey, gradient_magnitude);
      }
      std::cout<<"after inited msers"<<std::endl;
      {
        ...
        char buf[100]; sprintf(buf, "out1/%s.%d.%d.mser0.jpg", filename, step, iii);
          std::cout<<"display frame1"<<endl;
        //////attention: so emit cannot pass Mat data between threads?
        emit setFrameSig(1, &tmp);
        imwrite(buf, tmp);
      }
      {
        ...
        //else   std::cout<<"regions "<<ids[i].second<<" is kicked off"<<endl;
        regions = sr;
      }
      IFOPT{
        Mat tmp = Mat::zeros(img.size(), CV_8UC1);
      ...
      //t = cvGetTickCount() - t;
      //  std::cout << "Regions filled in " << t/((double)cvGetTickFrequency()*1000.) << " ms." << endl;
      ...
      vector<vector<bool> > sameline(N, vector<bool>(N, false));
      {
       ...
       std::cout<<"graph calculated"<<endl;
      ////////
      // graph, vis -> blocks
      ...
      vector<int> lineNum(N, -1);
      {
        ...
        for(int i=0; i<lines.size(); i++){
          ...
          int n = line.size();
          //   std::cout<<"line"<<i<<" : n="<< line.size()<<endl ;
          ////////
          ...
          if(n>1)
          {
            ...
            if((p.first>0.18 && p.second>0.3 || p.first>0.25 || p.second>0.5) && n<5 && lf.sw_var>0.01 ||
                ...
                std::cout<<"line"<<i<<" : n="<< line.size() <<", oy="<<p.first<<", ox="<<p.second<<endl;
              continue;
            }
            g/=n;
            ...
            if(f>=200 && g<=0.01 && h>=0.1 || f>120 && g<=0.01
                ...
                std::cout<<"#line"<<i<<" : n="<< line.size() <<", f="<<f<<", g="<<g<<", h="<<h<<", k="<<k<<endl;
              srt.push_back(line);
      ...
      // final_clusters, regions -> final_regions
        std::cout<<"store final regions"<<endl;
      {
        set<int> res;
      ...
      for(int i = 0; i< final_regions.size(); i++){
        ...
        if(!overlaps(final_regions[i], pixels[step-1], valid_num[step-1], k, c)) {
          //  std::cout<<"the region collapse with previous ones"<<endl;
          continue;
        }
        if(k>=0) {
        ...
        //   std::cout<<"the region is valid"<<endl;
...

      {
        ...
        cv::cvtColor(tmp, tmp, CV_GRAY2BGR);
          std::cout<<"display frame2"<<endl;
        //DisIMG(qf2, tmp);
        emit setFrameSig(2, &tmp);
        imwrite(buf, tmp);
      }
       QThread::msleep(300);
    ...
    //////////////////////////////////////////
    {
      ...
      cv::cvtColor(tmp, tmp, CV_GRAY2BGR);
        std::cout<<"display frame3"<<endl;
      //DisIMG(qf3, tmp);
      emit setFrameSig(3, &tmp);
      char buf[100];
